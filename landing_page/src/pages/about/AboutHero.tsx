...
export default function AboutHero() {
  return (
    <section className="pt-28 sm:pt-36 lg:pt-40 pb-16 sm:pb-20 bg-nexgen-pageLightBg overflow-hidden">
      <div className="qht-large-container">

        {/* Title with Inline Rounded Pill Image Capsules */}
        <div className="text-center max-w-4xl mx-auto mb-6">
          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-[500] text-nexgen-veryDarkHeader tracking-tight leading-[1.2] flex flex-col items-center justify-center">

            {/* Line 1: Redefining [pill 1] Hair */}
            <div className="flex items-center justify-center gap-2 sm:gap-3 flex-wrap">
              <span>Redefining</span>
              <span className="inline-flex items-center justify-center w-16 sm:w-20 md:w-22 h-8 sm:h-9 md:h-10 rounded-full overflow-hidden border border-nexgen-primaryGold/30 shadow-xs align-middle bg-white flex-shrink-0">
                <img
          ...
          {/* Subtitle */}
          <p className="mt-4 sm:mt-5 text-xs sm:text-[13.5px] text-gray-600 leading-relaxed max-w-2xl mx-auto font-normal text-center">
        ...
        <div className="max-w-5xl mx-auto rounded-3xl sm:rounded-[36px] overflow-hidden shadow-xl border border-nexgen-primaryGold/20 bg-white relative aspect-[16/10] sm:aspect-[1.85/1]">
          <img
        ...
        {/* Bottom Rounded Fixed-Width Stripe with Infinite Right-to-Left Marquee */}
        <div className="max-w-5xl mx-auto mt-6 rounded-2xl sm:rounded-full bg-nexgen-veryDarkHeader text-white py-4 px-4 sm:px-6 shadow-md border border-nexgen-primaryGold/30 overflow-hidden relative">
          <div className="animate-marquee flex items-center gap-8 whitespace-nowrap select-none text-xs sm:text-[13px] font-semibold">
            ...
            {STRIP_ITEMS.map((item, idx) => (
              <div key={`seq1-${idx}`} className="flex items-center gap-2 flex-shrink-0">
                <span className="text-nexgen-brightGold text-sm">✦</span>
                <span>{item}</span>
              </div>
            ))}
