...
import { authenticate, authorize, requirePlatformAdmin, AuthRequest } from "../middleware/authenticate";
import {
  DEPLOY_TARGETS,
  DEPLOY_ENABLED,
  DEPLOY_LOG_DIR,
...
/**
 * Manual backend deploys, triggered from Settings → Deploy (mirrors rhinon-cms).
 *
 * No SSH and no PEM key: the API already runs on the EC2 box as the `ubuntu` user,
 * so a deploy is a plain local `git pull && npm run build && pm2 restart`.
 *
 * Restarting kills THIS process mid-request, so the work is handed to a detached
 * child (scripts/deploy.sh) that outlives us, streams to a log file, and drops an
 * exit-code file when it finishes. Nothing here awaits the child — status is
 * reconstructed from those files on the next poll, which is what lets the flow
 * survive the API restarting underneath it.
 */
const router = Router();
router.use(authenticate);
// Restarting the server is Rhinon's job, not the clinic's. authorize() waves every
// superadmin through — including the clinic's own — so the permission check alone
// would not keep them out.
router.use(requirePlatformAdmin);

const DEPLOY_SCRIPT = path.join(__dirname, "..", "..", "scripts", "deploy.sh");
/** A run that has produced no exit file by now is assumed dead (script killed, box rebooted). */
  ...
  try {
    const targets = await Promise.all(
      Object.values(DEPLOY_TARGETS).map(async (t) => {
        const latest = await Deployment.findOne({ where: { target: t.key }, order: [["createdAt", "DESC"]] });
        return { ...publicTarget(t), latest: latest ? summarize(await finalize(latest)) : null };
      })
    );
...
router.post("/:target", authorize("deploy:trigger"), async (req: AuthRequest, res: Response) => {
  ...
  try {
    if (!fs.existsSync(path.join(target.repo, ".git"))) {
      res.status(400).json({ message: `No git checkout found on this server at ${target.repo}` });
      return;
    }
    ...
    if (inFlight && (await finalize(inFlight)).status === "running") {
      res.status(409).json({ message: `A ${target.label} deploy is already running.`, deploymentId: inFlight.id });
      return;
    }
    ...
    const runner = path.join(os.tmpdir(), `anwar-deploy-${dep.id}.sh`);
    ...
    console.log(`[deploy] ${target.key} started by ${req.user!.email} (${dep.id})`);
