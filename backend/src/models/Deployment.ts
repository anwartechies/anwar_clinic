...
// One row per deploy triggered from the admin panel — the build history (mirrors
// rhinon-cms). The live log lives on disk (see config/deployTargets.ts); it is
// copied into `log` when the run finishes so history survives log-dir cleanup.
export type DeploymentStatus = "running" | "success" | "failed";

interface DeploymentAttributes {
  id: string;
  /** Key from DEPLOY_TARGETS — never a client-supplied path. */
  target: string;
  status: DeploymentStatus;
  triggeredById: string | null;
  /** Denormalised so history still reads correctly after the account is removed. */
